...
try { (function () {
  ...
  function selectedEntries() {
    var cols = (state.collections && state.collections.length) ? state.collections : ["ty"];
  ...
  var AUDIO_MAX_ID = 300;  // 全部 300 条词条均有预生成音频
  var AUDIO_V = 1;         // 音频缓存版本：初版
  ...
  function renderMe() {
    ...
    COLLECTIONS.forEach(function (c) {
      ...
      html += '<div class="desc">' + (c.id === "ty" ? "已收录 300 条" : "即将上线") + "</div></div>";
